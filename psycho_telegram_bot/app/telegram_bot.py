    ...
    if response.status_code == 200:
        ...
        if not user_data:
            ...
            await update.message.reply_text(
                "Ваша подписка истекла. Пожалуйста, продлите ежемесячную подписку, чтобы продолжить использовать бота.")
            await update.message.reply_text(
                f"Переведите 1100 рублей по СБП на номер +79181051685 (Цифра банк) \n!ОБЯЗАТЕЛЬНО! c комментарием {user_data['comment']}. Платежи без комментария не возвращаются. Подписка ежемесячная./nПо вопросам подписки обращайтесь в Psychology help bot support(ссылка в профиле).")
            return
    else:
        await update.message.reply_text("Произошла ошибка. Пожалуйста, попробуйте позже.")
        return
